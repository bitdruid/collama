...
async function connError() {
    vscode.window.showErrorMessage(`Connection error to the ollama server @ ${userConfig.apiEndpointCompletion}`);
}
...
async function checkModelExists(): Promise<boolean> {
    try {
        ...
        if (sysConfig.backendCompletion === "ollama") {
            ...
            if (!modelExists) {
                vscode.window.showErrorMessage(
                    `Model ${userConfig.apiModelCompletion} does not exist on the Ollama server`,
                );
                return false;
            }
        ...
        if (sysConfig.backendCompletion === "openai") {
            ...
            if (!modelExists) {
                vscode.window.showErrorMessage(
                    `Model ${userConfig.apiModelCompletion} does not exist on the OpenAI server`,
                );
                return false;
            }
        ...
        // Unknown backend
        vscode.window.showErrorMessage(`Unsupported backend: ${sysConfig.backendCompletion}`);
        return false;
    } catch (error: unknown) {
        connError();
        return false;
